...
int main() {
  ...
  std::string encoded = base64_encode(
      reinterpret_cast<const unsigned char *>(orig.c_str()), orig.length());
  std::string decoded = base64_decode(encoded);

  if (encoded != "UmVuw6kgTnlmZmVuZWdnZXIKaHR0cDovL3d3dy5yZW5lbnlmZmVuZWdnZXIuY"
                 "2gKcGFzc2lvbiBmb3IgZGF0YQo=") {
  ...
  std::string encoded_mime_expeced = "b25lIHR3byB0aHJlZSBmb3VyIGZpdmUgc2l4IHNld"
                                     "mVuIGVpZ2h0IG5pbmUgdGVuIGVsZXZlbiB0\n"
                                     "d2VsdmUgdGhpcnRlZW4gZm91cnRlZW4gZml2dGVlb"
                                     "iBzaXh0ZWVuIHNldmVudGVlbiBlaWdodGVl\n"
  ...
  if (unpadded_decoded != "abcdefg") {
    std::cout << "Failed to decode unpadded input " << unpadded_input
              << std::endl;
    all_tests_passed = false;
  }
